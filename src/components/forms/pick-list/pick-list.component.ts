...
@Component({
  selector: 'pe-pick-list',
  templateUrl: './pick-list.component.html',
  styleUrls: ['./pick-list.component.css']
})

export class PickListComponent implements DoCheck, OnChanges {
  ...
  static DEFAULT_FORMAT = {
    all: 'All',
    none: 'None',
    direction: PickListComponent.LTR,
    locale: undefined
  };
  ...
  @Input() addAllLabel;
  @Input() removeAllLabel;
  @Input() draggable = true;
  @Input() id = `pick-list-${nextId++}`;
  @Input() key = '_id';
  @Input() display: any = '_name';
  @Input() height = '100px';
  @Input() filter = false;
  @Input() format = PickListComponent.DEFAULT_FORMAT;
  @Input() sort = false;
  ...
  constructor(private differs: IterableDiffers) {
    this.available = new BasicList(PickListComponent.AVAILABLE_LIST_NAME);
    this.confirmed = new BasicList(PickListComponent.CONFIRMED_LIST_NAME);
  }
  ...
  ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
    ...
    if (changeRecord['format']) {
      ...
      if (typeof (this.format.direction) === 'undefined') {
        this.format.direction = PickListComponent.LTR;
      }

      if (typeof (this.format.all) === 'undefined') {
        this.format.all = PickListComponent.DEFAULT_FORMAT.all;
      }

      if (typeof (this.format.none) === 'undefined') {
        this.format.none = PickListComponent.DEFAULT_FORMAT.none;
      }
    }
  ...
  direction() {
    return this.format.direction === PickListComponent.LTR;
  }
  ...
  moveAllItem(source: BasicList, target: BasicList, item: any = null, trueup = true) {
        ...
        const filtered = source.list.filter((item: any) => {
          if (Object.prototype.toString.call(item) === '[object Object]') {
            if (item._name !== undefined) {
              ...
              return item._name.toLocaleLowerCase(this.format.locale).indexOf(source.picker.toLocaleLowerCase()) !== -1;
            } else {
              // @ts-ignore: remove when d.ts has locale as an argument.
            ...
            // @ts-ignore: remove when d.ts has locale as an argument.
            return item.toLocaleLowerCase(this.format.locale).indexOf(source.picker.toLocaleLowerCase()) !== -1;
          }
        });
  ...
  eventEmit(action, form?: NgForm) {
    if (form) {
      this.formClicked.emit({data: this.confirmed, action: action, formValid: form.valid});
    } else {
      this.formClicked.emit({data: this.confirmed, action: action});
    }
  }
